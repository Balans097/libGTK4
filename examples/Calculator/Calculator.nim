...
# nim c -d:release Calculator.nim
...
proc onAddClicked(widget: GtkWidget; userData: pointer): bool =
  let
    textA: string = $gtk_editable_get_text(entryA)
    textB: string = $gtk_editable_get_text(entryB)

  var resultText: string
  try:
    let
      numA: float = parseFloat(textA)
      numB: float = parseFloat(textB)
    resultText = $(numA + numB)
  except ValueError:
    resultText = "Ошибка: введите числа"

  gtk_editable_set_text(entryResult, cstring(resultText))
  return true


# Обработчик активации приложения — формирование и отрисовка GUI
proc onActivate(app: GtkApplication; userData: pointer) =
  # Виджеты создаются в той же очерёдности расположения:
  # окно → текстовая метка → текстовые вводы → кнопка → текстовый вывод
  let window: GtkWindow = gtk_application_window_new(app)
  gtk_window_set_title(window, "Калькулятор сложения")
  gtk_window_set_default_size(window, 400, 300)

  let vbox: GtkBox = gtk_box_new(GtkOrientation.GTK_ORIENTATION_VERTICAL, 12)
  ...
  let labelTitle: GtkLabel = gtk_label_new("Введите два числа для сложения")
  ...
  let addButton: GtkButton = gtk_button_new_with_label("Сложить")
...
# Главная функция
proc main =
  let app: GtkApplication = gtk_application_new("org.example.calculator", G_APPLICATION_DEFAULT_FLAGS)
