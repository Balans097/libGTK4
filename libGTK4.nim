...
proc g_signal_handlers_unblock_by_func*(instance: gpointer, `func`: gpointer, data: gpointer): cuint {.inline.} =
  g_signal_handlers_unblock_matched(instance, cast[GSignalMatchType](ord(G_SIGNAL_MATCH_FUNC) or ord(G_SIGNAL_MATCH_DATA)),
                                     0, 0, nil, `func`, data)

proc g_signal_handlers_disconnect_by_func*(instance: gpointer, `func`: gpointer, data: gpointer): cuint {.inline.} =
  g_signal_handlers_disconnect_matched(instance, cast[GSignalMatchType](ord(G_SIGNAL_MATCH_FUNC) or ord(G_SIGNAL_MATCH_DATA)),
                                        0, 0, nil, `func`, data)

proc g_signal_handlers_disconnect_by_data*(instance: gpointer, data: gpointer): cuint {.inline.} =
  g_signal_handlers_disconnect_matched(instance, cast[GSignalMatchType](ord(G_SIGNAL_MATCH_DATA)),
                                        0, 0, nil, nil, data)
...
proc addClickGesture*(widget: GtkWidget, button = 1'u): GtkGestureClick =
  ...
  gtk_gesture_single_set_button(result, button.guint)
...
proc gtk_about_dialog_get_license_type*(about: pointer): cint  # реально: GtkLicense (enum) — было ошибочно pointer
proc gtk_about_dialog_get_wrap_license*(about: pointer): gboolean
proc gtk_about_dialog_get_system_information*(about: pointer): cstring
proc gtk_about_dialog_get_documenters*(about: pointer): pointer
proc gtk_about_dialog_get_artists*(about: pointer): pointer
proc gtk_about_dialog_get_translator_credits*(about: pointer): cstring
...
proc gtk_accessible_get_accessible_role*(self: pointer): cint  # реально: GtkAccessibleRole (enum) — было ошибочно pointer
proc gtk_accessible_list_get_type*(): GType
...
proc gtk_accessible_text_update_caret_position*(self: pointer)
proc gtk_accessible_text_update_selection_bound*(self: pointer)
proc gtk_action_bar_get_revealed*(action_bar: pointer): gboolean
proc gtk_adjustment_get_step_increment*(adjustment: pointer): gdouble
proc gtk_adjustment_get_page_increment*(adjustment: pointer): gdouble
proc gtk_adjustment_get_page_size*(adjustment: pointer): gdouble
proc gtk_adjustment_get_minimum_increment*(adjustment: pointer): gdouble
...
proc gtk_at_context_get_accessible_role*(self: pointer): cint  # реально: GtkAccessibleRole (enum) — было ошибочно pointer
proc gtk_at_spi_socket_get_bus_name*(self: pointer): cstring
proc gtk_at_spi_socket_get_object_path*(self: pointer): cstring
...
proc gtk_bitset_unref*(self: pointer)
proc gtk_bitset_is_empty*(self: pointer): gboolean
proc gtk_bitset_get_size*(self: pointer): guint64
proc gtk_bitset_get_minimum*(self: pointer): guint
proc gtk_bitset_get_maximum*(self: pointer): guint
proc gtk_bitset_new_empty*(): pointer
proc gtk_bitset_copy*(self: pointer): pointer
proc gtk_bitset_remove_all*(self: pointer)
proc gtk_bitset_iter_get_value*(iter: pointer): guint
proc gtk_bitset_iter_is_valid*(iter: pointer): gboolean
proc gtk_bookmark_list_get_filename*(self: pointer): cstring
proc gtk_bookmark_list_get_attributes*(self: pointer): cstring
proc gtk_bookmark_list_get_io_priority*(self: pointer): gint
proc gtk_bookmark_list_is_loading*(self: pointer): gboolean
proc gtk_bool_filter_new*(expression: pointer): pointer
proc gtk_bool_filter_get_expression*(self: pointer): pointer
proc gtk_bool_filter_get_invert*(self: pointer): gboolean
proc gtk_border_free*(border: pointer)
proc gtk_box_layout_new*(orientation: pointer): pointer
proc gtk_box_layout_get_homogeneous*(box_layout: pointer): gboolean
proc gtk_box_layout_get_spacing*(box_layout: pointer): guint
proc gtk_box_layout_get_baseline_position*(box_layout: pointer): cint  # реально: GtkBaselinePosition (enum) — было ошибочно pointer
proc gtk_box_layout_get_baseline_child*(box_layout: pointer): gint
proc gtk_buildable_get_buildable_id*(buildable: pointer): cstring
proc gtk_buildable_parse_context_pop*(context: pointer): pointer
proc gtk_buildable_parse_context_get_element*(context: pointer): cstring
proc gtk_builder_error_quark*(): GQuark
...
proc gtk_calendar_get_show_week_numbers*(self: pointer): gboolean
proc gtk_calendar_get_show_heading*(self: pointer): gboolean
proc gtk_calendar_get_show_day_names*(self: pointer): gboolean
proc gtk_calendar_get_day*(self: pointer): gint
proc gtk_calendar_get_month*(self: pointer): gint
proc gtk_calendar_get_year*(self: pointer): gint
proc gtk_calendar_get_date*(self: pointer): pointer
proc gtk_center_box_get_shrink_center_last*(self: pointer): gboolean
proc gtk_center_layout_new*(): pointer
proc gtk_center_layout_get_orientation*(self: pointer): cint  # реально: GtkOrientation (enum) — было ошибочно pointer
proc gtk_center_layout_get_baseline_position*(self: pointer): cint  # реально: GtkBaselinePosition (enum) — было ошибочно pointer
...
proc gtk_center_layout_get_shrink_center_last*(self: pointer): gboolean
proc gtk_color_dialog_button_new*(dialog: pointer): pointer
proc gtk_color_dialog_button_get_rgba*(self: pointer): pointer
proc gtk_color_dialog_get_title*(self: pointer): cstring
proc gtk_color_dialog_get_modal*(self: pointer): gboolean
proc gtk_color_dialog_get_with_alpha*(self: pointer): gboolean
...
proc gtk_column_view_column_get_id*(self: pointer): cstring
proc gtk_column_view_get_sorter*(self: pointer): pointer
proc gtk_column_view_get_tab_behavior*(self: pointer): cint  # реально: GtkListTabBehavior (enum) — было ошибочно pointer
...
proc gtk_column_view_row_get_accessible_description*(self: pointer): cstring
proc gtk_column_view_row_get_accessible_label*(self: pointer): cstring
proc gtk_column_view_sorter_get_primary_sort_column*(self: pointer): pointer
proc gtk_column_view_sorter_get_primary_sort_order*(self: pointer): cint  # реально: GtkSortType (enum) — было ошибочно pointer
proc gtk_column_view_sorter_get_n_sort_columns*(self: pointer): guint
proc gtk_constraint_is_constant*(constraint: pointer): gboolean
proc gtk_constraint_vfl_parser_error_quark*(): GQuark
...
proc gtk_css_provider_to_string*(provider: pointer): cstring
proc gtk_css_section_ref*(section: pointer): pointer
proc gtk_css_section_unref*(section: pointer)
proc gtk_css_section_to_string*(section: pointer): cstring
...
proc gtk_directory_list_get_attributes*(self: pointer): cstring
proc gtk_directory_list_get_io_priority*(self: pointer): gint
proc gtk_directory_list_is_loading*(self: pointer): gboolean
proc gtk_directory_list_get_error*(self: pointer): pointer
proc gtk_directory_list_get_monitored*(self: pointer): gboolean
...
proc gtk_drop_controller_motion_contains_pointer*(self: pointer): gboolean
proc gtk_drop_controller_motion_get_drop*(self: pointer): pointer
proc gtk_drop_controller_motion_is_pointer*(self: pointer): gboolean
proc gtk_drop_down_get_header_factory*(self: pointer): pointer
proc gtk_drop_down_get_search_match_mode*(self: pointer): cint  # реально: GtkStringFilterMatchMode (enum) — было ошибочно pointer
proc gtk_drop_target_async_get_formats*(self: pointer): pointer
proc gtk_drop_target_async_get_actions*(self: pointer): cint  # реально: GdkDragAction (enum) — было ошибочно pointer
proc gtk_editable_init_delegate*(editable: pointer)
proc gtk_editable_finish_delegate*(editable: pointer)
proc gtk_entry_get_overwrite_mode*(entry: pointer): gboolean
proc gtk_event_controller_get_current_event*(controller: pointer): pointer
proc gtk_event_controller_get_current_event_time*(controller: pointer): guint32
proc gtk_event_controller_get_current_event_device*(controller: pointer): pointer
proc gtk_event_controller_get_current_event_state*(controller: pointer): cint  # реально: GdkModifierType (enum) — было ошибочно pointer
proc gtk_expander_get_use_underline*(expander: pointer): gboolean
proc gtk_expander_get_use_markup*(expander: pointer): gboolean
proc gtk_expander_get_resize_toplevel*(expander: pointer): gboolean
proc gtk_expression_watch_ref*(watch: pointer): pointer
proc gtk_expression_watch_unref*(watch: pointer)
proc gtk_expression_watch_unwatch*(watch: pointer)
...
proc gtk_file_dialog_get_title*(self: pointer): cstring
proc gtk_file_dialog_get_modal*(self: pointer): gboolean
...
proc gtk_file_dialog_get_initial_name*(self: pointer): cstring
proc gtk_file_dialog_get_initial_file*(self: pointer): pointer
proc gtk_file_dialog_get_accept_label*(self: pointer): cstring
proc gtk_file_filter_new*(): pointer
proc gtk_file_filter_get_name*(filter: pointer): cstring
proc gtk_file_filter_add_pixbuf_formats*(filter: pointer)
...
proc gtk_file_launcher_get_always_ask*(self: pointer): gboolean
proc gtk_file_launcher_get_writable*(self: pointer): gboolean
proc gtk_filter_get_strictness*(self: pointer): cint  # реально: GtkFilterMatch (enum) — было ошибочно pointer
...
proc gtk_flow_box_child_is_selected*(child: pointer): gboolean
proc gtk_flow_box_child_changed*(child: pointer)
proc gtk_flow_box_get_activate_on_single_click*(box: pointer): gboolean
proc gtk_flow_box_remove_all*(box: pointer)
proc gtk_flow_box_select_all*(box: pointer)
proc gtk_flow_box_unselect_all*(box: pointer)
proc gtk_flow_box_invalidate_filter*(box: pointer)
proc gtk_flow_box_invalidate_sort*(box: pointer)
proc gtk_font_dialog_button_new*(dialog: pointer): pointer
proc gtk_font_dialog_button_get_dialog*(self: pointer): pointer
proc gtk_font_dialog_button_get_level*(self: pointer): cint  # реально: GtkFontLevel (enum) — было ошибочно pointer
proc gtk_font_dialog_button_get_font_desc*(self: pointer): pointer
proc gtk_font_dialog_button_get_font_features*(self: pointer): cstring
proc gtk_font_dialog_button_get_language*(self: pointer): pointer
proc gtk_font_dialog_button_get_use_font*(self: pointer): gboolean
proc gtk_font_dialog_button_get_use_size*(self: pointer): gboolean
proc gtk_font_dialog_new*(): pointer
proc gtk_font_dialog_get_title*(self: pointer): cstring
proc gtk_font_dialog_get_modal*(self: pointer): gboolean
...
proc gtk_gesture_pan_get_orientation*(gesture: pointer): cint  # реально: GtkOrientation (enum) — было ошибочно pointer
proc gtk_gesture_stylus_new*(): pointer
proc gtk_gesture_stylus_get_stylus_only*(gesture: pointer): gboolean
proc gtk_gesture_stylus_get_device_tool*(gesture: pointer): pointer
proc gtk_gl_area_get_allowed_apis*(area: pointer): cint  # реально: GdkGLAPI (enum) — было ошибочно pointer
proc gtk_gl_area_get_api*(area: pointer): cint  # реально: GdkGLAPI (enum) — было ошибочно pointer
proc gtk_gl_area_get_use_es*(area: pointer): gboolean
proc gtk_gl_area_get_auto_render*(area: pointer): gboolean
...
proc gtk_graphics_offload_get_enabled*(self: pointer): cint  # реально: GtkGraphicsOffloadEnabled (enum) — было ошибочно pointer
proc gtk_grid_layout_new*(): pointer
proc gtk_grid_layout_get_row_homogeneous*(grid: pointer): gboolean
proc gtk_grid_layout_get_row_spacing*(grid: pointer): guint
proc gtk_grid_layout_get_column_homogeneous*(grid: pointer): gboolean
proc gtk_grid_layout_get_column_spacing*(grid: pointer): guint
proc gtk_grid_layout_get_baseline_row*(grid: pointer): gint
proc gtk_grid_layout_child_get_row*(child: pointer): gint
proc gtk_grid_layout_child_get_column*(child: pointer): gint
proc gtk_grid_layout_child_get_column_span*(child: pointer): gint
proc gtk_grid_layout_child_get_row_span*(child: pointer): gint
proc gtk_grid_view_get_tab_behavior*(self: pointer): cint  # реально: GtkListTabBehavior (enum) — было ошибочно pointer
proc gtk_icon_paintable_get_file*(self: pointer): pointer
proc gtk_icon_paintable_get_icon_name*(self: pointer): cstring
proc gtk_icon_paintable_is_symbolic*(self: pointer): gboolean
...
proc gtk_icon_theme_get_theme_name*(self: pointer): cstring
...
proc gtk_image_clear*(image: pointer)
proc gtk_image_get_storage_type*(image: pointer): cint  # реально: GtkImageType (enum) — было ошибочно pointer
proc gtk_image_get_gicon*(image: pointer): pointer
proc gtk_image_get_icon_size*(image: pointer): cint  # реально: GtkIconSize (enum) — было ошибочно pointer
proc gtk_im_context_focus_in*(context: pointer)
proc gtk_im_context_focus_out*(context: pointer)
proc gtk_im_context_reset*(context: pointer)
proc gtk_im_multicontext_get_context_id*(context: pointer): cstring
...
proc g_unichar_isspace*(str: pointer): gboolean
proc gtk_label_get_label*(self: pointer): cstring
proc gtk_label_get_mnemonic_keyval*(self: pointer): guint
...
proc gtk_layout_manager_get_request_mode*(manager: pointer): cint  # реально: GtkSizeRequestMode (enum) — было ошибочно pointer
proc gtk_layout_manager_get_widget*(manager: pointer): pointer
proc gtk_layout_manager_layout_changed*(manager: pointer)
proc gtk_level_bar_get_mode*(self: pointer): cint  # реально: GtkLevelBarMode (enum) — было ошибочно pointer
proc gtk_level_bar_get_inverted*(self: pointer): gboolean
proc gtk_list_box_row_get_header*(row: pointer): pointer
proc gtk_list_box_row_changed*(row: pointer)
proc gtk_list_box_row_is_selected*(row: pointer): gboolean
proc gtk_list_box_row_get_selectable*(row: pointer): gboolean
proc gtk_list_box_row_get_activatable*(row: pointer): gboolean
proc gtk_list_box_remove_all*(box: pointer)
proc gtk_list_box_select_all*(box: pointer)
proc gtk_list_box_unselect_all*(box: pointer)
proc gtk_list_box_invalidate_filter*(box: pointer)
proc gtk_list_box_invalidate_sort*(box: pointer)
proc gtk_list_box_invalidate_headers*(box: pointer)
proc gtk_list_box_get_activate_on_single_click*(box: pointer): gboolean
proc gtk_list_box_drag_unhighlight_row*(box: pointer)
proc gtk_list_box_get_show_separators*(box: pointer): gboolean
proc gtk_list_header_get_item*(self: pointer): pointer
proc gtk_list_header_get_child*(self: pointer): pointer
proc gtk_list_item_get_accessible_description*(self: pointer): cstring
proc gtk_list_item_get_accessible_label*(self: pointer): cstring
proc gtk_list_view_get_header_factory*(self: pointer): pointer
proc gtk_list_view_get_tab_behavior*(self: pointer): cint  # реально: GtkListTabBehavior (enum) — было ошибочно pointer
proc gtk_map_list_model_get_model*(self: pointer): pointer
proc gtk_map_list_model_has_map*(self: pointer): gboolean
proc gtk_media_file_new_for_input_stream*(stream: pointer): pointer
proc gtk_media_stream_unprepared*(self: pointer)
proc gtk_media_stream_stream_unprepared*(self: pointer)
proc gtk_media_stream_ended*(self: pointer)
proc gtk_media_stream_stream_ended*(self: pointer)
proc gtk_media_stream_seek_success*(self: pointer)
proc gtk_media_stream_seek_failed*(self: pointer)
proc gtk_menu_button_get_label*(menu_button: pointer): cstring
proc gtk_menu_button_get_can_shrink*(menu_button: pointer): gboolean
proc gtk_mount_operation_get_type*(): GType
proc gtk_mount_operation_is_showing*(op: pointer): gboolean
...
proc gtk_native_realize*(self: pointer)
proc gtk_native_unrealize*(self: pointer)
proc gtk_native_get_for_surface*(surface: pointer): pointer
proc gtk_notebook_next_page*(notebook: pointer)
proc gtk_notebook_prev_page*(notebook: pointer)
proc gtk_notebook_get_show_border*(notebook: pointer): gboolean
proc gtk_notebook_get_scrollable*(notebook: pointer): gboolean
proc gtk_notebook_popup_enable*(notebook: pointer): pointer
proc gtk_notebook_popup_disable*(notebook: pointer)
proc gtk_orientable_get_orientation*(orientable: pointer): cint  # реально: GtkOrientation (enum) — было ошибочно pointer
proc gtk_overlay_layout_new*(): pointer
proc gtk_overlay_layout_child_get_measure*(child: pointer): gboolean
proc gtk_overlay_layout_child_get_clip_overlay*(child: pointer): gboolean
...
proc gtk_paned_get_resize_start_child*(paned: pointer): gboolean
proc gtk_paned_get_shrink_start_child*(paned: pointer): gboolean
proc gtk_paned_get_resize_end_child*(paned: pointer): gboolean
proc gtk_paned_get_shrink_end_child*(paned: pointer): gboolean
proc gtk_paned_get_wide_handle*(paned: pointer): gboolean
proc gtk_paper_size_free*(size: pointer)
proc gtk_paper_size_get_width*(size: pointer, unit: pointer): gdouble
proc gtk_paper_size_get_height*(size: pointer, unit: pointer): gdouble
proc gtk_paper_size_is_custom*(size: pointer): gboolean
proc gtk_paper_size_is_ipp*(size: pointer): gboolean
...
proc gtk_picture_get_content_fit*(self: pointer): cint  # реально: GtkContentFit (enum) — было ошибочно pointer
proc gtk_picture_get_alternative_text*(self: pointer): cstring
proc gtk_popover_get_position*(popover: pointer): cint  # реально: GtkPositionType (enum) — было ошибочно pointer
proc gtk_popover_get_autohide*(popover: pointer): gboolean
proc gtk_popover_get_has_arrow*(popover: pointer): gboolean
proc gtk_popover_get_mnemonics_visible*(popover: pointer): gboolean
proc gtk_popover_get_cascade_popdown*(popover: pointer): gboolean
proc gtk_popover_present*(popover: pointer)
...
proc gtk_popover_menu_get_flags*(popover: pointer): cint  # реально: GtkPopoverMenuFlags (enum) — было ошибочно pointer

# --------------------------------------------------------------------------
# SUPPORTS API
# --------------------------------------------------------------------------

proc gtk_print_setup_unref*(setup: pointer)
proc gtk_print_setup_get_print_settings*(setup: pointer): pointer
proc gtk_print_setup_get_page_setup*(setup: pointer): pointer
proc gtk_print_dialog_get_title*(self: pointer): cstring
proc gtk_print_dialog_get_accept_label*(self: pointer): cstring
proc gtk_print_dialog_get_modal*(self: pointer): gboolean
proc gtk_print_dialog_get_page_setup*(self: pointer): pointer
proc gtk_print_dialog_get_print_settings*(self: pointer): pointer
proc gtk_printer_get_name*(printer: pointer): cstring
proc gtk_printer_get_state_message*(printer: pointer): cstring
proc gtk_printer_get_description*(printer: pointer): cstring
proc gtk_printer_get_location*(printer: pointer): cstring
proc gtk_printer_get_icon_name*(printer: pointer): cstring
proc gtk_printer_get_job_count*(printer: pointer): gint
proc gtk_printer_is_active*(printer: pointer): gboolean
proc gtk_printer_is_paused*(printer: pointer): gboolean
proc gtk_printer_is_accepting_jobs*(printer: pointer): gboolean
proc gtk_printer_is_virtual*(printer: pointer): gboolean
proc gtk_printer_is_default*(printer: pointer): gboolean
proc gtk_printer_accepts_pdf*(printer: pointer): gboolean
proc gtk_printer_accepts_ps*(printer: pointer): gboolean
proc gtk_printer_has_details*(printer: pointer): gboolean
proc gtk_printer_request_details*(printer: pointer)
proc gtk_printer_get_capabilities*(printer: pointer): cint  # реально: GtkPrintCapabilities (enum) — было ошибочно pointer
proc gtk_print_job_get_title*(job: pointer): cstring
proc gtk_print_job_get_status*(job: pointer): cint  # реально: GtkPrintStatus (enum) — было ошибочно pointer
proc gtk_print_job_get_track_print_status*(job: pointer): gboolean
proc gtk_print_job_get_pages*(job: pointer): cint  # реально: GtkPrintPages (enum) — было ошибочно pointer
proc gtk_print_job_get_page_set*(job: pointer): cint  # реально: GtkPageSet (enum) — было ошибочно pointer
proc gtk_print_job_get_num_copies*(job: pointer): gint
proc gtk_print_job_get_scale*(job: pointer): gdouble
proc gtk_print_job_get_n_up*(job: pointer): guint
proc gtk_print_job_get_n_up_layout*(job: pointer): cint  # реально: GtkNumberUpLayout (enum) — было ошибочно pointer
proc gtk_print_job_get_rotate*(job: pointer): gboolean
proc gtk_print_job_get_collate*(job: pointer): gboolean
proc gtk_print_job_get_reverse*(job: pointer): gboolean
proc gtk_print_error_quark*(): pointer
proc gtk_print_operation_preview_end_preview*(preview: pointer)
proc gtk_print_settings_get_printer*(settings: pointer): pointer
proc gtk_print_settings_get_orientation*(settings: pointer): cint  # реально: GtkPageOrientation (enum) — было ошибочно pointer
proc gtk_print_settings_get_paper_size*(settings: pointer): pointer
proc gtk_print_settings_get_use_color*(settings: pointer): gboolean
proc gtk_print_settings_get_collate*(settings: pointer): gboolean
proc gtk_print_settings_get_reverse*(settings: pointer): gboolean
proc gtk_print_settings_get_duplex*(settings: pointer): cint  # реально: GtkPrintDuplex (enum) — было ошибочно pointer
proc gtk_print_settings_get_quality*(settings: pointer): cint  # реально: GtkPrintQuality (enum) — было ошибочно pointer
proc gtk_print_settings_get_n_copies*(settings: pointer): gint
proc gtk_print_settings_get_number_up*(settings: pointer): gint
proc gtk_print_settings_get_number_up_layout*(settings: pointer): cint  # реально: GtkNumberUpLayout (enum) — было ошибочно pointer
proc gtk_print_settings_get_resolution*(settings: pointer): gint
proc gtk_print_settings_get_resolution_x*(settings: pointer): gint
proc gtk_print_settings_get_resolution_y*(settings: pointer): gint
proc gtk_print_settings_get_printer_lpi*(settings: pointer): gdouble
proc gtk_print_settings_get_scale*(settings: pointer): gdouble
proc gtk_print_settings_get_print_pages*(settings: pointer): cint  # реально: GtkPrintPages (enum) — было ошибочно pointer
proc gtk_print_settings_get_page_set*(settings: pointer): cint  # реально: GtkPageSet (enum) — было ошибочно pointer
proc gtk_print_settings_get_default_source*(settings: pointer): cstring
proc gtk_print_settings_get_media_type*(settings: pointer): cstring
proc gtk_print_settings_get_dither*(settings: pointer): cstring
proc gtk_print_settings_get_finishings*(settings: pointer): cstring
proc gtk_print_settings_get_output_bin*(settings: pointer): cstring
proc gtk_print_unix_dialog_get_page_setup*(dialog: pointer): pointer
proc gtk_print_unix_dialog_get_current_page*(dialog: pointer): gint
...
proc gtk_print_unix_dialog_get_manual_capabilities*(dialog: pointer): cint  # реально: GtkPrintCapabilities (enum) — было ошибочно pointer
proc gtk_print_unix_dialog_get_support_selection*(dialog: pointer): gboolean
proc gtk_print_unix_dialog_get_has_selection*(dialog: pointer): gboolean
proc gtk_print_unix_dialog_get_embed_page_setup*(dialog: pointer): gboolean
proc gtk_print_unix_dialog_get_page_setup_set*(dialog: pointer): gboolean

# --------------------------------------------------------------------------
# SETLOCALE API
# --------------------------------------------------------------------------

proc gtk_progress_bar_get_pulse_step*(pbar: pointer): gdouble
proc gtk_progress_bar_get_inverted*(pbar: pointer): gboolean
proc gtk_progress_bar_get_ellipsize*(pbar: pointer): cint  # реально: PangoEllipsizeMode (enum) — было ошибочно pointer
proc gtk_range_get_adjustment*(range: pointer): pointer
proc gtk_range_get_inverted*(range: pointer): gboolean
proc gtk_range_get_flippable*(range: pointer): gboolean
proc gtk_range_get_slider_size_fixed*(range: pointer): gboolean
proc gtk_range_get_value*(range: pointer): gdouble
proc gtk_range_get_show_fill_level*(range: pointer): gboolean
proc gtk_range_get_restrict_to_fill_level*(range: pointer): gboolean
proc gtk_range_get_fill_level*(range: pointer): gdouble
proc gtk_range_get_round_digits*(range: pointer): gint
...
proc gtk_recent_info_unref*(info: pointer)
proc gtk_recent_info_get_uri*(info: pointer): cstring
proc gtk_recent_info_get_display_name*(info: pointer): cstring
proc gtk_recent_info_get_description*(info: pointer): cstring
proc gtk_recent_info_get_mime_type*(info: pointer): cstring
...
proc gtk_recent_info_get_private_hint*(info: pointer): gboolean
proc gtk_recent_info_get_gicon*(info: pointer): pointer
proc gtk_recent_info_get_age*(info: pointer): gint
proc gtk_recent_info_is_local*(info: pointer): gboolean
proc gtk_recent_info_exists*(info: pointer): gboolean
...
proc gtk_scale_button_get_active*(button: pointer): gboolean
proc gtk_scale_button_get_has_frame*(button: pointer): gboolean
proc gtk_scale_get_has_origin*(scale: pointer): gboolean
proc gtk_scale_get_layout*(scale: pointer): pointer
proc gtk_scale_clear_marks*(scale: pointer)
proc gtk_scrollable_get_hscroll_policy*(scrollable: pointer): cint  # реально: GtkScrollablePolicy (enum) — было ошибочно pointer
proc gtk_scrollable_get_vscroll_policy*(scrollable: pointer): cint  # реально: GtkScrollablePolicy (enum) — было ошибочно pointer
...
proc gtk_scrolled_window_unset_placement*(scrolled_window: pointer)
proc gtk_scrolled_window_get_placement*(scrolled_window: pointer): cint  # реально: GtkCornerType (enum) — было ошибочно pointer
proc gtk_scrolled_window_get_min_content_width*(scrolled_window: pointer): gint
proc gtk_scrolled_window_get_min_content_height*(scrolled_window: pointer): gint
proc gtk_scrolled_window_get_kinetic_scrolling*(scrolled_window: pointer): gboolean
proc gtk_scrolled_window_get_overlay_scrolling*(scrolled_window: pointer): gboolean
proc gtk_scrolled_window_get_max_content_width*(scrolled_window: pointer): gint
proc gtk_scrolled_window_get_max_content_height*(scrolled_window: pointer): gint
proc gtk_scrolled_window_get_propagate_natural_width*(scrolled_window: pointer): gboolean
proc gtk_scrolled_window_get_propagate_natural_height*(scrolled_window: pointer): gboolean
proc gtk_scroll_info_new*(): pointer
proc gtk_scroll_info_ref*(self: pointer): pointer
proc gtk_scroll_info_unref*(self: pointer)
proc gtk_scroll_info_get_enable_horizontal*(self: pointer): gboolean
proc gtk_scroll_info_get_enable_vertical*(self: pointer): gboolean
...
proc gtk_shortcut_action_to_string*(self: pointer): cstring
...
proc gtk_signal_action_get_signal_name*(self: pointer): cstring
proc gtk_named_action_new*(name: cstring): pointer
proc gtk_named_action_get_action_name*(self: pointer): cstring
proc gtk_shortcut_trigger_to_string*(self: pointer): cstring
proc gtk_shortcut_trigger_hash*(trigger: gconstpointer): guint
proc gtk_never_trigger_get*(): pointer
proc gtk_keyval_trigger_get_modifiers*(self: pointer): cint  # реально: GdkModifierType (enum) — было ошибочно pointer
proc gtk_keyval_trigger_get_keyval*(self: pointer): guint
proc gtk_mnemonic_trigger_new*(keyval: guint): pointer
proc gtk_mnemonic_trigger_get_keyval*(self: pointer): guint
...
proc gtk_slice_list_model_get_offset*(self: pointer): guint
proc gtk_slice_list_model_get_size*(self: pointer): guint
proc gtk_snapshot_gl_shader_pop_texture*(snapshot: pointer)
proc gtk_sorter_get_order*(self: pointer): cint  # реально: GtkSorterOrder (enum) — было ошибочно pointer
proc gtk_sort_list_model_get_section_sorter*(self: pointer): pointer
proc gtk_spin_button_get_activates_default*(spin_button: pointer): gboolean
proc gtk_spin_button_get_update_policy*(spin_button: pointer): cint  # реально: GtkSpinButtonUpdatePolicy (enum) — было ошибочно pointer
proc gtk_spin_button_get_numeric*(spin_button: pointer): gboolean
proc gtk_spin_button_get_wrap*(spin_button: pointer): gboolean
proc gtk_spin_button_get_snap_to_ticks*(spin_button: pointer): gboolean
proc gtk_spin_button_get_climb_rate*(spin_button: pointer): gdouble
proc gtk_spin_button_update*(spin_button: pointer)
proc gtk_spinner_get_spinning*(spinner: pointer): gboolean
proc gtk_stack_page_get_child*(self: pointer): pointer
proc gtk_stack_page_get_visible*(self: pointer): gboolean
proc gtk_stack_page_get_needs_attention*(self: pointer): gboolean
proc gtk_stack_page_get_use_underline*(self: pointer): gboolean
proc gtk_stack_page_get_name*(self: pointer): cstring
proc gtk_stack_page_get_title*(self: pointer): cstring
proc gtk_stack_page_get_icon_name*(self: pointer): cstring
proc gtk_stack_get_hhomogeneous*(stack: pointer): gboolean
proc gtk_stack_get_vhomogeneous*(stack: pointer): gboolean
proc gtk_stack_get_transition_running*(stack: pointer): gboolean
proc gtk_stack_get_interpolate_size*(stack: pointer): gboolean
...
proc gtk_string_sorter_get_collation*(self: pointer): cint  # реально: GtkCollation (enum) — было ошибочно pointer
...
proc gtk_test_widget_wait_for_draw*(widget: GtkWidget)
proc gtk_text_buffer_get_selection_content*(buffer: pointer): pointer
proc gtk_text_child_anchor_get_deleted*(anchor: pointer): gboolean
proc gtk_text_new*(): pointer
proc gtk_text_new_with_buffer*(buffer: pointer): pointer
proc gtk_text_get_visibility*(self: pointer): gboolean
proc gtk_text_get_invisible_char*(self: pointer): guint32
proc gtk_text_unset_invisible_char*(self: pointer)
proc gtk_text_get_overwrite_mode*(self: pointer): gboolean
proc gtk_text_get_max_length*(self: pointer): gint
proc gtk_text_get_text_length*(self: pointer): guint16
proc gtk_text_get_activates_default*(self: pointer): gboolean
proc gtk_text_get_placeholder_text*(self: pointer): cstring
proc gtk_text_get_input_purpose*(self: pointer): cint  # реально: GtkInputPurpose (enum) — было ошибочно pointer
proc gtk_text_get_input_hints*(self: pointer): cint  # реально: GtkInputHints (enum) — было ошибочно pointer
proc gtk_text_get_attributes*(self: pointer): pointer
proc gtk_text_get_tabs*(self: pointer): pointer
proc gtk_text_grab_focus_without_selecting*(self: pointer): gboolean
proc gtk_text_get_extra_menu*(self: pointer): pointer
proc gtk_text_get_enable_emoji_completion*(self: pointer): gboolean
proc gtk_text_get_propagate_text_width*(self: pointer): gboolean
proc gtk_text_get_truncate_multiline*(self: pointer): gboolean
proc gtk_text_iter_free*(iter: pointer)
proc gtk_text_iter_get_child_anchor*(iter: pointer): pointer
proc gtk_text_iter_get_language*(iter: pointer): pointer
proc gtk_text_iter_forward_visible_line*(iter: pointer): gboolean
proc gtk_text_iter_backward_visible_line*(iter: pointer): gboolean
proc gtk_text_view_reset_cursor_blink*(text_view: pointer)
proc gtk_tooltip_get_type*(): GType
...
proc gtk_tree_expander_get_indent_for_depth*(self: pointer): gboolean
proc gtk_tree_expander_get_indent_for_icon*(self: pointer): gboolean
proc gtk_tree_expander_get_hide_expander*(self: pointer): gboolean
proc gtk_tree_list_model_get_model*(self: pointer): pointer
proc gtk_tree_list_model_get_passthrough*(self: pointer): gboolean
proc gtk_tree_list_model_get_autoexpand*(self: pointer): gboolean
proc gtk_tree_list_row_get_item*(self: pointer): pointer
proc gtk_tree_list_row_get_expanded*(self: pointer): gboolean
proc gtk_tree_list_row_is_expandable*(self: pointer): gboolean
proc gtk_tree_list_row_get_position*(self: pointer): guint
proc gtk_tree_list_row_get_depth*(self: pointer): guint
...
proc gtk_uri_launcher_get_uri*(self: pointer): cstring
proc gtk_video_get_graphics_offload*(self: pointer): cint  # реально: GtkGraphicsOffloadEnabled (enum) — было ошибочно pointer
proc gtk_widget_unparent*(widget: GtkWidget)
proc gtk_widget_map*(widget: GtkWidget)
proc gtk_widget_unmap*(widget: GtkWidget)
...
proc gtk_widget_queue_resize*(widget: GtkWidget)
proc gtk_widget_queue_allocate*(widget: GtkWidget)
proc gtk_widget_get_frame_clock*(widget: GtkWidget): pointer
proc gtk_widget_get_request_mode*(widget: GtkWidget): cint  # реально: GtkSizeRequestMode (enum) — было ошибочно pointer
proc gtk_widget_get_layout_manager*(widget: GtkWidget): pointer
proc gtk_widget_class_get_layout_manager_type*(widget_class: pointer): GType
proc gtk_widget_class_get_activate_signal*(widget_class: pointer): guint
proc gtk_widget_activate*(widget: GtkWidget): gboolean
proc gtk_widget_get_focusable*(widget: GtkWidget): gboolean
proc gtk_widget_has_focus*(widget: GtkWidget): gboolean
proc gtk_widget_is_focus*(widget: GtkWidget): gboolean
proc gtk_widget_has_visible_focus*(widget: GtkWidget): gboolean
proc gtk_widget_get_focus_on_click*(widget: GtkWidget): gboolean
proc gtk_widget_get_can_target*(widget: GtkWidget): gboolean
proc gtk_widget_has_default*(widget: GtkWidget): gboolean
proc gtk_widget_get_receives_default*(widget: GtkWidget): gboolean
proc gtk_widget_get_state_flags*(widget: GtkWidget): pointer
proc gtk_widget_is_sensitive*(widget: GtkWidget): gboolean
proc gtk_widget_is_visible*(widget: GtkWidget): gboolean
proc gtk_widget_is_drawable*(widget: GtkWidget): gboolean
proc gtk_widget_get_realized*(widget: GtkWidget): gboolean
proc gtk_widget_get_mapped*(widget: GtkWidget): gboolean
proc gtk_widget_get_root*(widget: GtkWidget): pointer
proc gtk_widget_get_native*(widget: GtkWidget): pointer
proc gtk_widget_get_child_visible*(widget: GtkWidget): gboolean
proc gtk_widget_get_allocated_width*(widget: GtkWidget): gint
proc gtk_widget_get_allocated_height*(widget: GtkWidget): gint
proc gtk_widget_get_allocated_baseline*(widget: GtkWidget): gint
proc gtk_widget_get_width*(widget: GtkWidget): gint
proc gtk_widget_get_height*(widget: GtkWidget): gint
proc gtk_widget_get_baseline*(widget: GtkWidget): gint
proc gtk_widget_error_bell*(widget: GtkWidget)
proc gtk_widget_get_opacity*(widget: GtkWidget): gdouble
proc gtk_widget_get_overflow*(widget: GtkWidget): cint  # реально: GtkOverflow (enum) — было ошибочно pointer
proc gtk_widget_get_scale_factor*(widget: GtkWidget): gint
proc gtk_widget_get_settings*(widget: GtkWidget): pointer
proc gtk_widget_get_hexpand_set*(widget: GtkWidget): gboolean
proc gtk_widget_get_vexpand_set*(widget: GtkWidget): gboolean
proc gtk_widget_get_direction*(widget: GtkWidget): cint  # реально: GtkTextDirection (enum) — было ошибочно pointer
proc gtk_widget_set_default_direction*(dir: pointer)
proc gtk_widget_get_default_direction*(): cint  # реально: GtkTextDirection (enum) — было ошибочно pointer
proc gtk_widget_get_cursor*(widget: GtkWidget): pointer
proc gtk_widget_list_mnemonic_labels*(widget: GtkWidget): pointer
proc gtk_widget_trigger_tooltip_query*(widget: GtkWidget)
proc gtk_widget_get_has_tooltip*(widget: GtkWidget): gboolean
proc gtk_requisition_free*(requisition: pointer)
proc gtk_widget_in_destruction*(widget: GtkWidget): gboolean
proc gtk_widget_class_get_css_name*(widget_class: pointer): cstring
proc gtk_widget_init_template*(widget: GtkWidget)
proc gtk_widget_activate_default*(widget: GtkWidget)
...
proc gtk_widget_should_layout*(widget: GtkWidget): gboolean
proc gtk_widget_get_css_classes*(widget: GtkWidget): pointer
proc gtk_widget_class_get_accessible_role*(widget_class: pointer): cint  # реально: GtkAccessibleRole (enum) — было ошибочно pointer
proc gtk_widget_paintable_new*(widget: GtkWidget): pointer
proc gtk_widget_paintable_get_widget*(self: pointer): pointer
proc gtk_window_get_destroy_with_parent*(window: pointer): gboolean
proc gtk_window_get_hide_on_close*(window: pointer): gboolean
proc gtk_window_get_mnemonics_visible*(window: pointer): gboolean
proc gtk_window_get_focus_visible*(window: pointer): gboolean
proc gtk_window_is_active*(window: pointer): gboolean
proc gtk_window_get_icon_name*(window: pointer): cstring
...
proc gtk_window_has_group*(window: pointer): gboolean
proc gtk_window_is_maximized*(window: pointer): gboolean
proc gtk_window_is_suspended*(window: pointer): gboolean
proc gtk_window_set_interactive_debugging*(enable: gboolean)
proc gtk_window_get_handle_menubar_accel*(window: pointer): gboolean
...
proc isDarkTheme*(): bool =
  ...
  ## Читает булево GObject-свойство "gtk-application-prefer-dark-theme"
  ## у объекта настроек по умолчанию (GtkSettings) через универсальный
  ## g_object_get — отдельного getter'а для этого свойства в GTK4 нет.
  let settings = getDefaultSettings()

  # Настроек может не быть, если не инициализирован дисплей —
  # в этом случае считаем, что тёмная тема не используется
  if settings == nil:
    return false

  # g_object_get заполняет переданный по адресу буфер значением
  # свойства; список аргументов должен завершаться nil (аналог
  # NULL-терминатора в оригинальном C API)
  var preferDark: gboolean = 0
  g_object_get(toGObject(settings), "gtk-application-prefer-dark-theme", addr preferDark, nil)
